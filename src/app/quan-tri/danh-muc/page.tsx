...
const SmallTable = ({
  ...
  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
      ...
      <button className="text-xs text-brand-600 font-medium hover:underline">+ Thêm mới</button>
    </div>
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500">Trạng thái</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className={`border-t border-gray-50 ${i % 2 === 1 ? "bg-gray-50/50" : ""}`}>
              {row.slice(0, -1).map((cell, j) => (
                <td key={j} className="px-4 py-2.5 text-xs text-gray-700">{String(cell)}</td>
              ))}
