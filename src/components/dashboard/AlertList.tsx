        ...
        {alerts.map((alert) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] opacity-70 mt-0.5">{alert.product}</p>
              <p className="flex items-center gap-1 text-[10px] opacity-60 mt-1">
                <Clock size={9} />
                {alert.time}
              </p>
            </div>
            <span className="text-[10px] font-medium shrink-0">{levelLabel[alert.level]}</span>
          </div>
        ))}
      </div>
      <Link
        href="/rui-ro/canh-bao"
        className="mt-4 flex items-center justify-center gap-1 text-xs text-brand-600 font-semibold hover:text-brand-700 transition-colors"
      >
