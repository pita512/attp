        ...
        {activities.map((a) => (
          <div key={a.id} className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-gray-700 truncate">{a.action}</p>
              <p className="text-[11px] text-gray-400 truncate">{a.entity}</p>
            </div>
            <p className="text-[10px] text-gray-400 shrink-0 whitespace-nowrap">{a.time}</p>
          </div>
        ))}
      </div>
    </div>
